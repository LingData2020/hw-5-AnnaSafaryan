...
```
Если мы рассматриваем f2 в завистимости от f1 и vowels, то коэффициенты получаются больше у двух параметров, связанных с vowels -- причём у vowelA коэффициент по модулю даже больше, чем у f1, когда мы рассматривали f2 только в зависимости от f1. Можно сделать вывод, что f2 больше зависит от vowelA и vowely, чем от f1. Несмотря на это, R^2 хоть и повысилась с 0.33 до 0.48, качество предсказания даже с учётом vowels всё ещё не очень хорошее. 
```
...
```
Хи-квадрат можно применять, только если ожидаемая частота > 5. Переменная с такими частотами только одна - CeSynt. Её будем проверять через точный текст Фишера
```
```{r 3.1}
...
```
Связь между Aux и  CeSynt, измеренная тестом Фишера, является статистически значимой, т.к. p-value < 0.05
```

### 3.2. 
Test the hypothesis that `Aux` and `EPTrans` are not independent with the help of Pearson's Chi-squared Test. 
```{r 3.2}
auxept <- chisq.test(d_caus$Aux, d_caus$EPTrans)
auxept
```
```
p-value < 0.05, значит, переменные зависимы
```
...
```
odds ratio для ожидаемых частот = 1, что соотвествует независимым данным.
```
...
```{r 3.5}
library(lsr)
cramersV(auxept$observed)
```

### 3.6. 
Report the results of independence test using the following template:
```
We have found a significant association between variables Aux and EPTrans (p < 0.001). The odds of "doen" were 2.6 times higher in intransitive predicates than in transitive. Effect size is small (Cramer's V = 0.17).
```
...
```
Чем больше раз мы тестируем гипотезы отдельно, тем больше накапливается вероятность ошибки.
```

### 3.9 
Provide a short text (300 words) describing the hypothesis of this study and the results of your analysis.
```
В голландском языке есть два вспомогательных глагола: doen и laten. Гипотеза исследования заключается в том, что выбор глагола зависит от переходности сказуемого. После применения теста хи-квадрат выяснилось, что между переменными "вспомогательный глагол" и "переходность" действительно есть статистически значимая связь. Шанс встретить "doen" у непереходного глагола в 2,6 раз выше, чем у переходного, однако, это хоть и статситически значимое, но небольшое отклонение от H0, т.к Сramer's V = 0.17.
```
